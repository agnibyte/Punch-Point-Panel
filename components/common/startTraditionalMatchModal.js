...
export default function StartTraditionalMatchModal({
  pendingMatches,
  refereePath,
}) {
  ...
  const onClickContinue = () => {
    console.log("selectedMatch", selectedMatch, refereePath);
    // Save the selected match number and timer duration in localStorage
    localStorage.setItem("currentMatch", selectedMatch.value);
    router.push(
      `${refereePath}?participant=${selectedMatch.name}&state=${selectedMatch.state}`
    ); // Redirect to the referee path
  };
